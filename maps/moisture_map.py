...
def generate_moisture_map(static_dir):
    # Load soil moisture data
    moisture_collection = (ee.ImageCollection('NASA_USDA/HSL/soil_moisture')
                           .filterDate(START_DATE, END_DATE)
                           .filterBounds(ee.Geometry.Polygon(FRANCE)))
    soil_moisture = moisture_collection.select('ssm').mean()
    ...
    for idx, tile in enumerate(TILES):
        url = soil_moisture.getThumbURL({
            'region': ee.Geometry.Polygon([tile]),
            'dimensions': '512x512',  # Smaller dimensions for each tile
            'format': 'png',
            'min': moisture_vis_params['min'],
            'max': moisture_vis_params['max'],
            'palette': moisture_vis_params['palette']
        })
        urls.append(url)
        print(f'URL de l\'image d\'humidité du sol pour la tuile {idx + 1} : {url}')

    # Download images with progress bar
    for idx, url in enumerate(tqdm(urls, desc="Téléchargement des tuiles d'humidité du sol")):
        response = session.get(url)
        png_path = os.path.join(static_dir, f'moisture_tile_{idx + 1}.png')
        with open(png_path, 'wb') as file:
            file.write(response.content)

    # Combine the images in the correct order
    order = [1, 2, 7, 4, 3, 8, 6, 5, 9]
    tiles_images = [Image.open(os.path.join(static_dir, f'moisture_tile_{i}.png')) for i in order]
    ...
    plt.axis('off')
    legend_image_path = os.path.join(static_dir, 'moisture_map_with_legend.png')
